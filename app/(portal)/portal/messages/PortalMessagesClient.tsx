...
export default function PortalMessagesClient() {
  ...
  const selectConversation = (id: string) => {
    setActiveId(id);
    router.replace(`/portal/messages?c=${id}`);
  };
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const res = await fetch(`/api/portal/messages/${activeId}`, {
        method: "POST",
      ...
      >
        ...
        <div className="pm-thread" style={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
          {!activeId ? (
            ...
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: "1px solid var(--cc-border)", flexShrink: 0 }}>
                <Avatar name={thread.conversation.org.name} src={thread.conversation.org.logoUrl ?? undefined} size="sm" />
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "var(--cc-text)" }}>{thread.conversation.org.name}</div>
                  {thread.conversation.campaign && (
                    <div style={{ fontSize: 12, color: "var(--cc-text-muted)" }}>{thread.conversation.campaign.title}</div>
                  )}
                </div>
              </div>
                      ...
                      <div style={{ maxWidth: "72%" }}>
                        ...
                        <div
                          style={{
                            ...
                            background: isCreator ? "var(--cc-primary)" : "var(--cc-bg)",
                            color: isCreator ? "#fff" : "var(--cc-text)",
                            border: isCreator ? "none" : "1px solid var(--cc-border)",
                          }}
                        >
                          {m.body}
                        </div>
                        <div style={{ fontSize: 10, color: "var(--cc-text-subtle)", marginTop: 3, textAlign: isCreator ? "right" : "left" }}>
